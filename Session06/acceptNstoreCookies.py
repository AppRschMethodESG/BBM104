...
topics = [#'Investor',
    'sustainability', 'ESG', 'environment', 'social', 'governance', 'corporate responsibility', 'transparency'] 
...
async def acceptNstoreCookies():
    ...
    try:
        ...
        # Save the URLs to urls_raw.csv
        with open('urls_raw.csv', 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['URL'])
            writer.writerows([[url] for url in urls])


        # Filter URLs based on the specified topics (see the list at the beginning of the script)
        filteredUrls = [url for url in urls if any(re.search(keyword.replace('_', '[_-]'), url, re.IGNORECASE) for keyword in topics)]
        # Save the filtered URLs to urls_filtered.csv
        with open('urls_filtered.csv', 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['URL'])
